...
(async () => {
  ...
  const { composePlanInstruction, composeReviewInstruction } = await import(
    resolve(cwd, 'dist/compose.js')
  );

  // composePlanInstruction
  const plan = composePlanInstruction('Improve DX', 'No breaking changes', [
    'src/server.ts',
    'README.md',
  ]);
