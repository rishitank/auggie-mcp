...
void __dirname; // ensure lint doesn't flag unused in ESM context
...
const sanitizeMaybePath = (p?: string): string | undefined => {
  if (!p) return p;
...
const startHttpStreamingServer = () => {
    ...
    if (req.method === 'POST' && req.url === '/stream') {
      if (process.env.AUGGIE_MCP_ALLOW_EXEC !== 'true') {
        ...
        res.end(
          JSON.stringify({
            error: 'Forbidden',
            code: 'EXEC_DISABLED',
            hint: 'Set AUGGIE_MCP_ALLOW_EXEC=true',
          }),
        );
        return;
      }
      ...
      req.on('end', () => {
        try {
          ...
          const { args, stdinText: stdinTextRaw } = parsed as {
            args?: unknown;
            stdinText?: unknown;
          };
          ...
          const child = spawn('auggie', argsVal!, {
            cwd: REPO_ROOT,
  ...
  server.listen(port, () => {
    // logging is helpful here for local development
...
const main = async () => {
  ...
  server.tool(
    'auggie_call',
    { args: z.array(z.string()).describe('Arguments after the `auggie` command') },
    ...
    async ({ args }) => {
      if (process.env.AUGGIE_MCP_ALLOW_EXEC !== 'true') {
        return {
          content: [
            {
              type: 'text',
              text: 'Execution disabled. Set AUGGIE_MCP_ALLOW_EXEC=true to allow auggie_call.',
            },
          ],
        };
      }
      try {
        ...
        return {
          content: [{ type: 'text', text: `Execution failed: ${e.stderr || e.err?.message || e}` }],
        };
      }
    },
  );
  ...
  server.tool(
    'project_search_md',
    { query: z.string(), maxResults: z.number().int().positive().max(500).optional() },
  ...
  server.tool(
    ...
    async ({ instruction, quiet, compact, rulesFile, mcpConfig }) => {
      ...
      if (rulesAbs) args.push('--rules', rulesAbs);
      if (mcpAbs) args.push('--mcp-config', mcpAbs);
  ...
  server.tool(
    'auggie_continue',
    { quiet: z.boolean().optional(), compact: z.boolean().optional() },
    async ({ quiet, compact }) => {
      const args = ['--continue'];
      if (quiet) args.push('--quiet');
      if (compact) args.push('--compact');
  ...
  server.tool(
    'auggie_run_file',
    { path: z.string(), quiet: z.boolean().optional(), compact: z.boolean().optional() },
    async ({ path: rel, quiet, compact }) => {
  ...
  server.tool(
    'auggie_auth',
    { action: z.enum(['login', 'logout', 'print-token']) },
    async ({ action }) => {
  ...
  server.tool('git_status', async () => {
    try {
      ...
      return {
        content: [{ type: 'text', text: `git_status failed: ${e.stderr || e.err?.message || e}` }],
      };
    }
  });
  ...
  server.tool(
    'git_diff',
    { refA: z.string().optional(), refB: z.string().optional(), path: z.string().optional() },
    async ({ refA, refB, path: p }) => {
      try {
        const args = ['diff'];
        ...
        return {
          content: [{ type: 'text', text: `git_diff failed: ${e.stderr || e.err?.message || e}` }],
        };
      }
    },
  );
  ...
  // Prompt: hello
  server.prompt('hello', 'Returns a friendly greeting', async () => {
    return {
      messages: [
  ...
  server.prompt(
    'strategy_of_attack',
    'Drafts a high-level plan to integrate Auggie CLI capabilities via MCP',
    async () => {
      return {
        messages: [
          {
  ...
  server.prompt(
    'commit_message',
    'Generates a conventional commit message from summary + context',
    { summary: z.string(), details: z.string().optional(), scope: z.string().optional() },
    async ({ summary, details, scope }) => {
      ...
      return {
        messages: [
          {
            role: 'assistant',
            content: { type: 'text', text: `feat${scopeStr}: ${summary}${body}` },
          },
        ],
      };
    },
  ...
  server.prompt(
    'pr_description',
    'Drafts a PR description from title + changes',
    { title: z.string(), changes: z.string() },
    async ({ title, changes }) => {
      const text = `# ${title}\n\n## Summary\n${changes}\n\n## Checklist\n- [ ] Tests pass\n- [ ] Lint passes\n- [ ] Docs updated`;
  ...
  server.resource('version', 'app://version', async () => {
    return {
      contents: [{ uri: 'app://version', mimeType: 'text/plain', text: 'auggie-mcp 0.1.0' }],
    };
  });
