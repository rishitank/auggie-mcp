...
export const findFilesByExt = async (root: string, exts: string[]): Promise<string[]> => {
...
export const findMarkdownFiles = async (): Promise<string[]> => findFilesByExt(REPO_ROOT, ['.md']);

export const searchInMdFiles = async (
  query: string,
  maxResults = 50,
...
export const searchInText = async (
  query: string,
  globs: string[] | undefined,
  maxResults = 200,
): Promise<Array<{ path: string; line: number; preview: string }>> => {
  const files = await findFilesByExt(REPO_ROOT, [
    '.md',
    '.ts',
    '.tsx',
    '.js',
  ...
  const filtered =
    globs && globs.length ? files.filter((f) => globs.some((g) => f.includes(g))) : files;
  const out: Array<{ path: string; line: number; preview: string }> = [];
