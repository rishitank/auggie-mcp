export const composePlanInstruction = (goal: string, constraints?: string, paths?: string[]) => {
  const pathList =
    paths && paths.length ? `\n\nPaths in focus:\n${paths.map((p) => `- ${p}`).join('\n')}` : '';
  return [
...
export const composeReviewInstruction = (title?: string, paths?: string[], hasDiff?: boolean) => {
  ...
  return (
    ...
    [
      ...
      filesList,
      hasDiff
        ? 'Context comes from stdin as a unified diff.'
        : 'If stdin provides file content, treat each section as a separate file context.',
    ]
      .filter(Boolean)
      .join('\n\n')
  );
